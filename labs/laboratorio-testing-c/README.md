...
> R: Inicializa el carrito en 0. Es importante llamarla antes de usarlo para que el programa empiece vacio y no trabaje con valores indefinidos de memoria.
...
> R:No pasa. Esperaba 700 y da 350
...
> R:tiene que mmultiplicar por cantidad
...
> R:falla. el quinto carrito_agregar devuelve 1.
...
> R:=< es incorrecto y el correcto es solo <.
...
> R:carrito_descuento porque no fue ejecutada por los tests de cobertura. 
...
> R:el unitario prueba de forma aislada. el de integracion las prueba funcionando juntas. el bug de carrito_total lo detecto primero el test unitario.
...
> R:porque en c no hay control automatico y puede esribir en memoria que no le corresponde. en python y java si existe y lanza un error.
...
> R:porque los tests detectan automaticamente lo que uno podria no ver en el funcionamiento normal o solo leyendo el codigo.
...
> R:porque con un solo producto, no cambia multiplicar por la cantidad porque es lo mismo. pero cuando la cantidad es 2 no es lo mismo y cambia el valor esperado.
